...
function Contactus() {
  ...
  useEffect(() => {
    ...
    const fetchProperties = async () => {
      try {
        const response = await axios.get('http://localhost:4000/api/Allcustom_box');
    ...
    const handleDelete = async (propertyId) => {
          ...
          if (result.isConfirmed) {
            try {
              ...
              await axios.delete(`http://localhost:4000/api/Allboxs/${propertyId}`);
      ...
      {properties.map((property) => (
        <tr key={property.id}>
          ...
          <td className="text-nowrap">
                    ...
                    <img src={property.img}  alt={property.img}   style={{ width: '100px', height: '100px', objectFit: 'cover' }} 
